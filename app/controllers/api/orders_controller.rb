  ...
  def index
    if params.has_key?(:status)
      if params[:status] == 'archived'
        render json: @user.orders.where('datetime < ? OR status not in (?)', 30.minutes.since(Time.zone.now), [:pending, :accepted])
                         .includes(:place).pag(@offset, @limit),
               each_serializer: ClientOrderSerializer
      elsif params[:status] == 'current'
        render json: @user.orders.where('datetime > ? AND status in (?)', 30.minutes.since(Time.zone.now), [:pending, :accepted])
                   .includes(:place).pag(@offset, @limit),
               each_serializer: ClientOrderSerializer
      else
